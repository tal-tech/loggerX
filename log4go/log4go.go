...
var (
	levelStrings = [...]string{"FNST", "FINE", "DEBG", "INFO", "WARN", "EROR", "TRAC", "CRIT"}
)
...
type LogRecord struct {
	...
	//    Added by xueersi
	// used for identify `x_timestamp` field which uses cache time by default
	useTrace       bool
	traceOptionals map[string]string
}
...
var g_traceVersion = "NotSet"

func SetTraceVersionParam(version string) {
	g_traceVersion = version
}

var g_traceDepartment = "NotSet"

func SetTraceDepartment(department string) {
	g_traceDepartment = department
}
...
func (log Logger) LogTrace(lvl Level, source, message string, traceMeta *map[string]string) {
	...
	rec := &LogRecord{
		...
		useTrace:       true,
		traceOptionals: make(map[string]string),
	}
	for key, value := range *traceMeta {
		rec.traceOptionals[key] = value
	}
	rec.traceOptionals["x_department"] = `"` + g_traceDepartment + `"`
	rec.traceOptionals["x_version"] = `"` + g_traceVersion + `"`
